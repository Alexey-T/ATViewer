...
begin
  Result:=0;
  h:=OpenFile(PAnsiChar(AnsiString(FileName)), Struct, OF_SHARE_DENY_NONE);
  try
...
var
  ...
  lHandle:Cardinal;
begin
  lHandle:=FindFirstFileA(PAnsiChar(AnsiString(S)), lFindData);
...
var
  BinaryType:DWord;
begin
  if GetBinaryTypeA(PAnsiChar(AnsiString(FileName)), BinaryType) then begin
