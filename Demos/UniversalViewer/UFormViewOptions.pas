...
uses
  ...
  StdCtrls, ComCtrls, ATxToolbarList, ExtCtrls, TntDialogs, Spin,
  ecHotKeyEdit;
...
type
  TFormViewOptions = class(TForm)
    ...
    edIgnore: TEdit;
    chkIgnore: TCheckBox;
    chkToolbarFS: TCheckBox;
    chkRen: TCheckBox;
    btnTextColorHi: TButton;
    Timer1: TTimer;
    chkImageFitWidth: TCheckBox;
    chkImageFitHeight: TCheckBox;
    btnKeyReset: TButton;
    HotKey1: TecHotKey;
    ...
    procedure edHTextKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure chkHTextClick(Sender: TObject);
    procedure btnTextColorHiClick(Sender: TObject);
    procedure Timer1Timer(Sender: TObject);
    procedure btnKeyResetClick(Sender: TObject);
  private
  ...
  public
    ...
    ffTextHiColor: TColor;
    ffTextDetect: boolean;
    ffTextDetectOEM: boolean;
    ffTextDetectUTF8: boolean;
...
uses
  ATViewer, ATViewerMsg, ATxSProc, ATxParamStr,
  ATxMsgProc, ATxMsg,
...
begin
  with TFormViewOptionsText.Create(Self) do
    try
      ...
      chkDetectUTF8.Checked:= ffTextDetectUTF8;
      ...
      if ShowModal=mrOk then
        ...
        ffTextDetectUTF8:= chkDetectUTF8.Checked;
...
begin
  with ListKeys do
    ...
    btnKeyReset.Enabled:= btnKeyOk.Enabled;
    HotKey1.Enabled:= btnKeyOk.Enabled;
    if Assigned(Selected) then
      if ffToolbar.GetAvail(integer(Selected.Data), Rec) then
      begin
        HotKey1.HotKey:= Rec.FMenuItem.Shortcut;
        HotKey1.Enabled:= Rec.FEn;
        btnKeyOk.Enabled:= HotKey1.Enabled;
        btnKeyReset.Enabled:= btnKeyOk.Enabled;
      end;
    end;
end;
...
begin
  ...
  with ListKeys do
    for i:= 0 to Items.Count-1 do
      ...
      begin
        MsgError(Format(MsgString(317), [Items[i].Caption]), Handle);
        Exit;
      end;
...
procedure TFormViewOptions.edHTextKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Key=vk_escape) and (Shift=[]) then btnCancel.Click;
  if (Key=vk_return) and (Shift=[]) then btnOk.Click;
end;

procedure TFormViewOptions.chkHTextClick(Sender: TObject);
begin
...
procedure TFormViewOptions.btnTextColorHiClick(Sender: TObject);
begin
  with ColorDialog1 do
    begin
    Color:= ffTextHiColor;
...
procedure TFormViewOptions.Timer1Timer(Sender: TObject);
begin
  Timer1.Enabled:= false;
  if FPlugin = 'slister' then
  begin
    SendToBack;
    BringToFront;
  end;
...
procedure TFormViewOptions.btnKeyResetClick(Sender: TObject);
begin
  HotKey1.HotKey:= 0;
  btnKeyOkClick(Self);
end;
